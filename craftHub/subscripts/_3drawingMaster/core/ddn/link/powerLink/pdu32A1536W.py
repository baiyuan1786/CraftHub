...
class PDU_32A1536W(PDUConnectionPanel):
    "32A输入，1536W标准PDU, 适合大多数情况"
    ...
    def __init__(self, doc: Drawing, installPnum: str, isNew: bool) -> None:
        ...
        for insertList in [LEFT_X_LIST, RIGHT_X_LIST]:
            for index, x in enumerate(insertList, start = 1):
                if index < 3:
                    self._addTerminal(x=x, y=self.OUT_MOD_INTERVAL_Y, index=index, a="16A")
                else:
    ...
    def _addTerminal(self, x: float, y: float, index: int, a: Literal["16A", "10A"]):
        '''插入某个安数的端子，使用正中点插入'''
