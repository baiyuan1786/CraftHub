##########################################################################################################
#   Description: 纵向加密连接图本期新增成端IDF设备节点
...
class CryptoNewEdgedIDF(CommonCryptoDev):
    '''纵向加密连接图本期新增成端IDF设备节点'''

    DEVICE_NAME = "本期新增成端IDF"
    DEVICE_TYPE = CryptoDeviceType.NEW_EDGED_IDF
    IS_NEW = True
    ...
    def __init__(
            self,
            deviceNum: str,
            portR: str = DEFAULT_PORT_R,
            portNR: str = DEFAULT_PORT_NR,
            isRoom2: bool = False,
            isJump: bool = False,
