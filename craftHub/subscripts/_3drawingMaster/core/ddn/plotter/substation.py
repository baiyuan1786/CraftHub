##########################################################################################################
#   Description: ddn定向式绘图网络绘图器， 站绘图器
...
class DDNsubplotter:
    ...
    LEFT_FRAME_OFFSET = Vec2(370, 0)                                         # 左图偏置
    RIGHT_FRAME_OFFSET = Vec2(841, 0) + Vec2(38.4559, 0) + LEFT_FRAME_OFFSET # 右图偏置
    SUBSTATION_NAME_OFFSET_Y = Vec2(0, 142)                                  # 站名纵向偏置
    ...
    def _buildStation(self, data: DataUnitDDN):
        '''已建设站点构建'''
        ...
        # 参数校准
        # ddn和idn设备非同屏安装时，不显示idn设备占位信息
        if data.get("DDNInstallPnum") != data.get("IDNInstallPnum"):
            data.set("IDNAltitudeU", None)
            data.set("IDNisNewPDU", False)
            data.set("IDNPDUAltitudeU", None)

        # idnPDU不是新增时，不显示idnPDU安装高度
        if not data.get("IDNisNewPDU"):
            data.set("IDNPDUAltitudeU", None)

        # ddnPDU不是新增时，不显示ddnPDU安装高度
    ...
    def _notBuildStation(self, data: DataUnitDDN):
        '''未建设站点构建'''
    ...
    def _addSubstationNameText(self, layout: Modelspace, insertPoint: Vec2):
        ...
        if not self.data.get("build"):
            text += f"({CADColor.colored('规划建设中', '黄色')})"
    ...
    def insertInto(self, layout: Modelspace, insertPoint: Vec2):
        ...
        self.leftPlotter.insertInto(
            layout,
            insertPoint + self.LEFT_FRAME_OFFSET
        )

        self.rightPlotter.insertInto(
            layout,
            insertPoint + self.RIGHT_FRAME_OFFSET
        )
