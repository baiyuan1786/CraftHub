...
class IDNsubPlotter_right:
    ...
    def addFrameCabinet(self):
        ...
        if isinstance(self.frame, FrameA3plus):
            self.frame.grid(FrameCabinetA3plus(doc=self.doc), self.frame.frameCabinetPoint())
    ...
    def addIntroduction(self):
        ...
        self.frame.grid(A3plusIntroduction(doc=self.doc), self.frame.introductionPoint())

    def addUsedCabinet(self, data: DataUnitIDN, insertPoint: Vec2):
        '''增加被使用的屏柜类到图框中'''
        ...
        cabinet = Cabinet(
            pNum=data.get("installPnum"),
            name=data.get("installPName"),
            cabinetType=data.get("installCabinetType"),
        )

        existedDeviceList: List[ExistedDevice] = [
            ExistedDevice(rawName, altitude, height)
            for rawName, altitude, height in zip(deviceNameList, deviceAltitudeUList, deviceHeightUList)
        ]

        calculator = AutoUcalculator(existedDeviceList=existedDeviceList)
        calculator.installPDU(altitudeU=data.get("PDUAltitudeU"))
        calculator.installPDU(altitudeU=data.get("DDNPDUAltitudeU"))
        calculator.installIDN(altitudeU=data.get("altitudeU"))
        ...
        self.frame.grid(cabinet.toPanel(doc=self.doc), insertPoint)

    def addNewDevicePanel(self, data: DataUnitIDN, insertPoint: Vec2):
        '''增加新增设备面板图到图框中'''

        device = IDN设备(altitudeU=data.get("altitudeU"))
        self.frame.grid(device.toDevicePanel(doc=self.doc), insertPoint)
