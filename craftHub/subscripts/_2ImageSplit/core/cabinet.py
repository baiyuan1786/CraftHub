...
class Cabinet:
    ...
    def parseName(cls, 
        ...
        # 进行过滤
        filtedCandidates = []
        for c in cabinetCandidates:
            maxLen = CABINET_LEN_MAX if "(" not in c else CABINET_LEN_MAX + 10
            
            if len(c) > maxLen: # 长度过滤
                continue
            if "-" in c or "—" in c:    # 字符过滤
                continue
            
            filtedCandidates.append(c)
        
        cabinetCandidates = filtedCandidates
        if not cabinetCandidates:
            selected = None
