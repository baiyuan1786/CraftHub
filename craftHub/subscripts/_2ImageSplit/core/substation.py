...
class Substaion(QObject):
    ...
    SAME_ROOM_THR = 60 * 10
    sitePattern = re.compile(r'^((?:1[0-9]{2}|[2-4][0-9]{2}|500|1[0-9]|[2-9][0-9]|10))([kK][vV])([\u4e00-\u9fa5a-zA-Z0-9]+?)(?:变电|站|站|变|巡维|运维)')
    ...
    def parseName(cls, 
        ...
        for i in range(min(cls.REC_NAME_MAX_COUNT, len(datedImages))):
            ...
            if not result:
                continue
            result = [t.replace("千伏", "kV").replace("站", "站") for t in result]
